...
/**
 * محصول خام دریافتی از ووکامرس (REST API v3) را به تایپ Product داخلی پروژه
 * تبدیل می‌کند تا هیچ کامپوننتی (ProductCard، صفحه‌ی محصول و...) با فیلد
 * گم‌شده یا ناسازگار خطا ندهد.
 *
 * ⚠️ نکته‌ی مهم درباره‌ی قیمت: اگر واحد پول فروشگاه ووکامرس شما «ریال» است
 * نه «تومان»، قیمت را قبل از نمایش باید بر ۱۰ تقسیم کنید. پایین همین فایل
 * یک پرچم TOMAN_DIVIDER برای این کار گذاشته شده.
 */

const TOMAN_DIVIDER = 10; // اگر فروشگاه‌تان قیمت را به ریال ذخیره می‌کند، این را به 10 تغییر دهید
...
export function adaptProduct(raw: any): Product {
  ...
  const stockQty = raw?.stock_quantity != null ? toNumber(raw.stock_quantity, 0) : undefined;
  ...
  return {
    ...
    scale: extractAttribute(raw, "مقیاس") ?? extractAttribute(raw, "scale") ?? "استاندارد",
    sku: raw?.sku ?? "",
    ...
    description: raw?.description ? splitParagraphs(stripHtml(raw.description) ?? "") : [],
    specs: Array.isArray(raw?.attributes)
      ? raw.attributes
          .filter((a: any) => a?.name && Array.isArray(a?.options))
          .map((a: any) => ({ label: a.name, value: a.options.join("، ") }))
      : [],
    image: images[0],
    images,
...
function extractAttribute(raw: any, name: string): string | undefined {
  const attr = raw?.attributes?.find((a: any) => a?.name === name);
